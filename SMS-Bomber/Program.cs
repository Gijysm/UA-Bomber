...
internal class Program
{
    static async Task Main()
    {
        ...
        for (; ; )
        {
            ...
            if (taskBatch.Count >= batchSize)
            {
                try
                {
                    ...
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Sent batch of {taskBatch.Count} requests");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Error in batch: {ex.Message}");
                }
                taskBatch.Clear();
            }
        ...
        {
            await Task.WhenAll(taskBatch);
            Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] Sent remaining {taskBatch.Count} requests");
        }
